...
| **Upstream impact** | Three defects in `find_inconsistent_windings.py` found by the audit: a candidate patch for the two solver defects, failing before and passing after on villa `main` as of 20 September 2026 (`23c3d759`); the third is fixed in villa `main` by [villa#1864](https://github.com/ScrollPrize/villa/pull/1864), merged on 25 September 2026 |
...
| **Upstream report** | [ScrollPrize/villa#1855](https://github.com/ScrollPrize/villa/issues/1855), with the real-data reproduction and the candidate patch. Two community pull requests implement the fixes on top of it and reproduce the results from this repository, with upstream unit tests in both: [villa#1864](https://github.com/ScrollPrize/villa/pull/1864), the attachment-step correction, **merged into villa `main` on 25 September 2026**, and [villa#1876](https://github.com/ScrollPrize/villa/pull/1876), which carries this repository's solver patch unchanged |
...
**The attachment gap is never transported** — the defect described above. It is corrected and tested in the patch-graph harness (`WIDE_ATTACHMENT_GAP=1`), and [WIDE_SCANSPACE_AUDIT.md](WIDE_SCANSPACE_AUDIT.md) specifies the change in terms that carry over to upstream's code. That change is now in villa `main`: [villa#1864](https://github.com/ScrollPrize/villa/pull/1864), merged on 25 September 2026, reproduces this repository's corrected graph with 0 differences over all 1,304 directed edges.
